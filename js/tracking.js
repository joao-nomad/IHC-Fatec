...
function (t) {
    ...
    tracking.Image = {}, tracking.Image.blur = function (t, r, n, e) {
        ...
        for (var s = 0; n > s; s++) for (var g = 0; r > g; g++) {
            var h = s * r * 4 + 4 * g,
        ...
        for (var i = new Uint8ClampedArray(e ? t.length : t.length >> 2), a = 0, o = 0, c = 0; n > c; c++) for (var s = 0; r > s; s++) {
            var g = .299 * t[o] + .587 * t[o + 1] + .114 * t[o + 2];
            i[a++] = g, e && (i[a++] = g, i[a++] = g, i[a++] = t[o + 3]), o += 4
        }
        return i
    }, tracking.Image.horizontalConvolve = function (t, r, n, e, i) {
        for (var a = e.length, o = Math.floor(a / 2), c = new Float32Array(r * n * 4), s = i ? 1 : 0, g = 0; n > g; g++) for (var h = 0; r > h; h++) {
            for (var k = g, u = h, f = 4 * (g * r + h), l = 0, p = 0, v = 0, m = 0, y = 0; a > y; y++) {
                var d = k,
                    w = Math.min(r - 1, Math.max(0, u + y - o)),
                    T = 4 * (d * r + w),
                    _ = e[y];
                l += t[T] * _, p += t[T + 1] * _, v += t[T + 2] * _, m += t[T + 3] * _
            }
    ...
    tracking.ViolaJones = {}, tracking.ViolaJones.REGIONS_OVERLAP = .5, tracking.ViolaJones.classifiers = {}, tracking.ViolaJones.detect = function (t, r, n, e, i, a, o, c) {
        ...
        for (var l = c[0], p = c[1], v = e * i, m = v * l | 0, y = v * p | 0; r > m && n > y;) {
            for (var d = v * a + .5 | 0, w = 0; n - y > w; w += d) for (var T = 0; r - m > T; T += d) o > 0 && this.isTriviallyExcluded(o, s, w, T, r, m, y) || this.evalStages_(c, k, u, f, w, T, r, m, y, v) && (h[g++] = {
                width: m,
                height: y,
                x: T,
                y: w
        ...
        for (var y = t.length, d = 2; y > d;) {
            for (var w = 0, T = t[d++], _ = t[d++]; _--;) {
                for (var C = 0, E = t[d++], M = t[d++], I = 0; M > I; I++) {
                    var x, b, O, A, S = a + t[d++] * g + .5 | 0,
                        R = i + t[d++] * g + .5 | 0,
                        D = t[d++] * g + .5 | 0,
                        j = t[d++] * g + .5 | 0,
                        F = t[d++];
                    E ? (x = S - j + D + (R + D + j - 1) * o, b = S + (R - 1) * o, O = S - j + (R + j - 1) * o, A = S + D + (R + D - 1) * o, C += (e[x] + e[b] - e[O] - e[A]) * F) : (x = R * o + S, b = x + D, O = x + j * o, A = O + D, C += (r[x] - r[b] - r[O] + r[A]) * F)
                }
                var L = t[d++],
                    V = t[d++],
                    U = t[d++];
        ...
        for (var r = new tracking.DisjointSet(t.length), n = 0; n < t.length; n++) for (var e = t[n], i = 0; i < t.length; i++) {
            var a = t[i];
    ...
    tracking.Fast = {}, tracking.Fast.THRESHOLD = 40, tracking.Fast.circles_ = {}, tracking.Fast.findCorners = function (t, r, n, e) {
        ...
        for (var c = 3; n - 3 > c; c++) for (var s = 3; r - 3 > s; s++) {
    ...
    tracking.Matrix = {}, tracking.Matrix.forEach = function (t, r, n, e, i) {
        i = i || 1;
        for (var a = 0; n > a; a += i) for (var o = 0; r > o; o += i) {
            var c = a * r * 4 + 4 * o;
    ...
    tracking.ColorTracker = function (t) {
    ...
    }, tracking.ColorTracker.prototype.colors = ["magenta"], tracking.ColorTracker.prototype.minDimension = 20, tracking.ColorTracker.prototype.minGroupSize = 30, tracking.ColorTracker.prototype.calculateDimensions_ = function (t, r) {
        ...
        for (var r, n = [], e = this.getMinDimension(), i = 0; i < t.length; i++) {
            var a = t[i];
            r = !0;
            for (var o = i + 1; o < t.length; o++) {
                ...
                if (tracking.Math.intersectRect(a.x, a.y, a.x + a.width, a.y + a.height, c.x, c.y, c.x + c.width, c.y + c.height)) {
                    r = !1;
                    var s = Math.min(a.x, c.x),
                        g = Math.min(a.y, c.y),
                        h = Math.max(a.x + a.width, c.x + c.width),
                        k = Math.max(a.y + a.height, c.y + c.height);
                    c.height = k - g, c.width = h - s, c.x = s, c.y = g;
                    break
                }
            }
            r && a.width >= e && a.height >= e && n.push(a)
        }
        return n
        ...
        var o, c, s, g, h, k = tracking.ColorTracker.knownColors_[a],
            ...
            y = -4;
        if (!k) return m;
        for (var d = 0; i > d; d++) for (var w = 0; e > w; w++) if (y += 4, !f[y]) {
            for (o = 0, h = -1, v[++h] = y, v[++h] = d, v[++h] = w, f[y] = 1; h >= 0;) if (s = v[h--], c = v[h--], g = v[h--], k(n[g], n[g + 1], n[g + 2], n[g + 3], g, c, s)) {
                u[o++] = s, u[o++] = c;
                for (var T = 0; T < p.length; T++) {
                    var _ = g + p[T],
                        C = c + t[T],
                        E = s + r[T];
                    !f[_] && C >= 0 && i > C && E >= 0 && e > E && (v[++h] = _, v[++h] = C, v[++h] = E, f[_] = 1)
                }
            }
            if (o >= l) {
